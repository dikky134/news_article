    ...
    return (
        <MainLayout activePage="trending">
            ...
            <main className="max-w-content-max-width mx-auto px-margin-edge py-section-gap bg-surface dark:bg-[#121212] dark:text-white border-outline-variant dark:border-zinc-800">
                <header className="mb-16 border-b border-outline-variant pb-8">
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                        <div>
                            ...
                            <h2 className="font-headline-md text-headline-md md:font-display-xl md:text-headline-lg lg:text-display-xl mb-2">
                                Trending Now
                            </h2>
                ...
                {/* Container Utama: Menggunakan flex-col di mobile agar 'order' bekerja, kembali ke grid asli di desktop */}
                <div className="flex flex-col md:grid md:grid-cols-12 gap-gutter">
                    
                    {/* TOP TRENDING #01: Berada di paling atas saat mobile (order-1) */}
                    {mainFeature && (
                        <article className="order-1 md:order-none md:col-span-8 group mb-gutter md:mb-0">
                            <Link href={`/articles/${mainFeature.slug}`} className="cursor-pointer block">
                                ...
                                <div className="flex flex-col gap-4">
                                    ...
                                    <h2 className="font-headline-md text-headline-md md:font-headline-lg md:text-headline-lg group-hover:text-secondary dark:group-hover:text-amber-500 transition-colors leading-tight uppercase">
                                        {mainFeature.title}
                                    </h2>
                                    <p className="text-on-surface-variant dark:text-on-primary-container font-body-lg max-w-3xl line-clamp-2">
                                        {mainFeature.excerpt}
                                    </p>
                                    <div className="dark:text-on-primary-container flex flex-wrap items-center gap-x-6 gap-y-2 mt-4">
                    ...
                    {/* PILLAR LIST #02 - #04: Berada di bawah konten utama saat mobile (order-2) */}
                    <section className="order-2 md:order-none md:col-span-4 border-r-0 md:border-r border-outline-variant pr-0 md:pr-gutter pb-gutter md:pb-0 border-b md:border-b-0 mb-gutter md:mb-0">
                            ...
                            {leftPillar.map((item, index) => (
                                ...
                                >
                                    <div className="flex items-start gap-4">
                                        <span className="font-headline-md text-primary dark:text-on-secondary opacity-20 dark:opacity-80 text-headline-md md:font-display-xl md:text-headline-lg leading-none">
                ...
                <div className="mt-section-gap grid grid-cols-1 md:grid-cols-3 gap-gutter">
                    {bentoRow.map((item, index) => (
                        <Link 
                            key={item.id} 
                            href={`/articles/${item.slug}`} 
                            className="p-6 bg-surface-container-lowest dark:bg-inverse-surface border border-outline-variant hover:shadow-sm transition-all duration-300 group cursor-pointer"
                        >
                            <span className="font-headline-md text-primary dark:text-on-secondary opacity-50 text-headline-md md:font-display-xl mb-2 block">
                                {String(index + 5).padStart(2, '0')}
                            </span>
                            <span className="text-secondary dark:text-amber-500 font-label-caps mb-2 block uppercase">{item.category?.name}</span>
                            <h4 className="font-headline-md text-headline-md mb-4 leading-snug group-hover:text-secondary dark:group-hover:text-amber-500 transition-colors uppercase">
                                {item.title}
                            </h4>
                            <p className="text-on-surface-variant dark:text-on-secondary text-body-md mb-6 line-clamp-3">
                                {item.excerpt}
                            </p>
                            <div className="flex justify-between items-center border-t border-outline-variant pt-4">
                                ...
                                <span className="material-symbols-outlined text-primary dark:text-on-secondary">arrow_forward</span>
                            </div>
                        </Link>
                    ))}
                </div>
                ...
                <section className="mt-section-gap bg-primary text-on-primary p-margin-edge">
                    <div className="flex flex-col md:flex-row justify-between items-center gap-8">
                        <div className="text-center md:text-left">
                            <h3 className="font-headline-md md:font-headline-lg mb-2 uppercase italic">Inside the Numbers</h3>
                            <p className="opacity-70 font-body-md italic">Real-time data from our global readership network.</p>
                        </div>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-8 md:gap-12 w-full md:w-auto">
                            {dynamicStats.map((stat, i) => (
                                <div key={i} className="text-center">
                                    <span className="font-headline-md block text-headline-md md:font-display-xl md:text-headline-lg">{stat.val}</span>

...
export default function ArticleDetail({ article, auth, relatedArticles = [] }) {
    ...
    return (
        <MainLayout activePage={article.category?.slug}>
            ...
            {showToast && (
                <div className="fixed top-24 right-5 z-50 flex items-center w-full max-w-xs p-4 space-x-4 text-gray-500 bg-white rounded-xl shadow-2xl dark:text-gray-400 dark:bg-zinc-950 border border-emerald-100 dark:border-emerald-950/30 transition-all duration-500 ease-in-out transform translate-y-0 opacity-100">
                    <div className="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 text-emerald-500 bg-emerald-100 rounded-lg dark:bg-emerald-900/30 dark:text-emerald-400">
                        <span className="material-symbols-outlined text-[20px]">check_circle</span>
                    </div>
                    <div className="flex-1">
                        <h4 className="text-sm font-semibold text-slate-800 dark:text-white">Sukses!</h4>
                        <p className="text-xs text-slate-500 dark:text-zinc-400 mt-0.5">{flash.success}</p>
                    </div>
                    <button 
                        onClick={() => setShowToast(false)}
                        className="text-gray-400 hover:text-gray-900 dark:hover:text-white rounded-lg p-1.5 inline-flex items-center justify-center h-8 w-8 transition-colors"
                    >
                        ...
                        <div className="flex flex-col gap-8 text-slate-500 dark:text-zinc-400">
                            ...
                            {auth?.user?.id === 1 && (
                                <>
                                    ...
                                    <Link 
                                        href={`/articles/${article.id}/edit`} // Cocok dengan rute /articles/{article}/edit di web.php

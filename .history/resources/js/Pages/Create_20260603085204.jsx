...
export default function Create({ categories = [] }) {
    ...
    // State tambahan untuk mengelola Kategori Baru
            ...
            <main className="max-w-content-max-width mx-auto px-4 md:px-margin-edge py-12 w-full overflow-x-hidden">
                        ...
                        <div className="bg-surface-container-low dark:bg-zinc-900 p-6 md:p-8 border border-outline-variant space-y-6 md:space-y-8">
                            ...
                            {showManageCategory && (
                                <div className="p-4 border border-dashed border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900/50 rounded-md">
                                    <h4 className="text-xs font-bold uppercase tracking-wider mb-3 text-zinc-500">Quick Category Manager</h4>
                        ...
                        <div className="border border-outline-variant p-4 md:p-6 space-y-4 w-full">
                            ...
                            <div className="group bg-white dark:bg-zinc-900 p-4 shadow-sm border border-outline-variant/30 w-full">
                                ...
                                <h4 className="font-headline-md text-base md:text-[18px] leading-tight mb-2 dark:text-white uppercase line-clamp-2 tracking-tight">
                                    {/* Judul biasanya tidak mengandung HTML, jika ada, Anda bisa menerapkan fungsi strip HTML juga */}
                                ...
                                <p className="font-body-md text-xs md:text-[13px] text-on-surface-variant line-clamp-2 leading-relaxed">
                                    {data.subheadline ? (
                                        ...
                                        // REGEX KUNCI: Menghilangkan semua tag HTML secara real-time saat user mengetik
                ...
                {isLinkModalOpen && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                        {/* Backdrop / Latar Belakang Blur Gelap */}
                        <div 
                    ...
                    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                        ...
                        <div className="relative bg-white dark:bg-zinc-900 border border-outline-variant dark:border-zinc-800 p-6 md:p-8 w-full max-w-md shadow-2xl animate-in fade-in zoom-in-95 duration-150">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                <button type="button" onClick={() => setIsCategoryModalOpen(false)} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 text-lg font-medium cursor-pointer">✕</button>
                            </div>

                            {/* Form Tambah Kategori Baru */}
                            <form onSubmit={handleAddCategory} className="mb-6">
                                <label className="block font-label-caps text-[10px] tracking-widest text-zinc-500 dark:text-zinc-400 uppercase font-bold mb-2">Create New Category</label>
                                <div className="flex gap-2">
                                    <input 
                                        type="text" 
                                        required
                                        placeholder="e.g., Politics, Technology..."
                                        className="flex-1 text-sm border border-zinc-300 dark:border-zinc-700 bg-transparent p-2.5 text-slate-800 dark:text-zinc-100 focus:outline-none focus:border-secondary dark:focus:border-amber-500 uppercase text-xs tracking-wider"
                                        value={newCategoryName}
                                        onChange={(e) => setNewCategoryName(e.target.value)}
                                    />
                                    <button type="submit" className="bg-zinc-900 text-white dark:bg-zinc-100 dark:text-zinc-900 px-4 text-xs font-label-caps uppercase tracking-wider font-bold hover:bg-secondary dark:hover:bg-amber-500 dark:hover:text-zinc-900 transition-colors cursor-pointer">
                                        Add
                                    </button>
                                </div>
                                {categoryError && <p className="text-red-500 text-xs mt-1.5 font-medium">{categoryError}</p>}
                            </form>

                            {/* List Kategori yang Ada saat ini */}
                            <div>
                                <label className="block font-label-caps text-[10px] tracking-widest text-zinc-500 dark:text-zinc-400 uppercase font-bold mb-2">Existing Categories ({categories.length})</label>
                                <div className="max-h-48 overflow-y-auto space-y-1.5 border border-zinc-200 dark:border-zinc-800 p-2 bg-zinc-50 dark:bg-zinc-900/40">
                                    {categories.length === 0 ? (
                                        <p className="text-xs text-center text-zinc-400 py-4 italic">No categories created yet.</p>
                                    ) : (
                                        categories.map(cat => (
                                            <div key={cat.id} className="flex justify-between items-center text-xs p-2 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 shadow-sm">
                                                <span className="uppercase tracking-wider font-medium text-slate-700 dark:text-zinc-300">{cat.name}</span>
                                                <button 
                                                    type="button" 
                                                    onClick={() => initiateDeleteCategory(cat)}
                                                    className="text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 p-1 rounded transition-colors flex items-center justify-center cursor-pointer"
                            ...
                            <div className="flex justify-end pt-5 mt-4 border-t border-zinc-100 dark:border-zinc-800">
                                <button 
                                    ...
                                    className="px-5 py-2 text-xs font-label-caps uppercase tracking-wider font-bold bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:opacity-90 cursor-pointer"
                                >
                                    Close
                                </button>
                            </div>
                ...
                {isDeleteConfirmOpen && (
                    <div className="fixed inset-0 z-[210] flex items-center justify-center p-4">
                        ...
                        <div className="relative bg-white dark:bg-zinc-900 border border-red-200 dark:border-red-900/40 p-6 md:p-8 w-full max-w-sm shadow-2xl animate-in fade-in zoom-in-95 duration-100">
                            <div className="text-center space-y-3">
                                ...
                                <h3 className="font-['Newsreader'] italic text-xl font-semibold text-primary dark:text-zinc-100">
                                    Delete Category?
                                </h3>
                                
                                <p className="text-sm text-zinc-500 dark:text-zinc-400 leading-relaxed">
                                    Are you sure you want to permanently delete <strong className="uppercase text-red-600 dark:text-red-400">"{categoryToDelete?.name}"</strong>? This action cannot be undone.
                                </p>
                            </div>
                            ...
                            <div className="flex gap-3 mt-6">
                                <button 
                                    ...
                                    className="flex-1 px-4 py-2.5 text-xs font-label-caps uppercase tracking-wider font-bold border border-zinc-300 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors cursor-pointer text-center"
                                >
                                    Cancel
                                </button>
                                <button 
                                    ...
                                    className="flex-1 bg-red-600 text-white dark:bg-red-500 px-4 py-2.5 text-xs font-label-caps uppercase tracking-wider font-bold hover:opacity-90 transition-opacity cursor-pointer text-center shadow-sm"
                                >
                                    Delete Now
                                </button>
                            </div>

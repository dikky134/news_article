...
export default function Trending({ trendingArticles = [], stats }) {
    ...
    useEffect(() => {
        // Ambil waktu saat komponen pertama kali di-mount di sisi client (Aman dari Hydration Mismatch)
    ...
    const getImageUrl = (thumbnail) => {
        if (!thumbnail) return '/images/placeholder.jpg'; // Tambahkan placeholder jika gambar kosong
        return thumbnail.startsWith('http') ? thumbnail : `/storage/${thumbnail}`;
    };

    // Membagi data artikel
    const mainFeature = trendingArticles[0]; 
    const leftPillar = trendingArticles.slice(1, 3); 
    const bentoRow = trendingArticles.slice(3, 7); 
            ...
            <main className="max-w-content-max-width mx-auto px-margin-edge py-section-gap bg-surface dark:bg-[#121212] dark:text-white border-outline-variant dark:border-zinc-800">
                ...
                {trendingArticles.length === 0 ? (
                    <div className="text-center py-16 border border-dashed border-outline-variant rounded-lg">
                        <p className="text-on-surface-variant dark:text-on-primary-container font-body-lg">
                            No trending articles available at the moment. Check back later!
                        </p>
                    </div>
                ) : (
                    <>
                        ...
                        <div className="flex flex-col md:grid md:grid-cols-12 gap-gutter">
                                    ...
                                    <Link href={`/articles/${mainFeature.slug}`} className="cursor-pointer block">
                                        <div className="relative mb-4 md:mb-6 overflow-hidden">
                                            <img 
                                                ...
                                                src={getImageUrl(mainFeature.thumbnail)} 
                                            />
                            ...
                            {/* PILLAR LIST #02 - #04 */}
                            {leftPillar.length > 0 && (
                                <section className="order-2 md:order-none md:col-span-4 border-l-0 md:border-l border-outline-variant pl-0 md:pl-gutter pb-6 md:pb-0 border-b md:border-b-0 mb-6 md:mb-0">

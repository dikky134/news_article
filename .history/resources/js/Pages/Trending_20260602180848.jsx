    ...
    return (
        <MainLayout activePage="trending">
            ...
            <main className="max-w-content-max-width mx-auto px-margin-edge py-section-gap bg-surface dark:bg-[#121212] dark:text-white border-outline-variant dark:border-zinc-800">
                {/* Perbaikan Kotak & Teks Header Mobile */}
                <header className="mb-8 md:mb-16 border-b border-outline-variant pb-6 md:pb-8">
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-6">
                        <div>
                            <span className="bg-surface-container-highest text-primary font-label-caps px-2 py-1 mb-4 md:mb-6 inline-block text-xs uppercase">
                                Weekly Report
                            </span>
                            <h2 className="text-3xl font-bold md:font-display-xl md:text-headline-lg lg:text-display-xl mb-2">
                                Trending Now
                            </h2>
                            <p className="text-on-surface-variant dark:text-on-primary-container text-base md:font-body-lg max-w-2xl">
                ...
                <div className="flex flex-col md:grid md:grid-cols-12 gap-gutter">
                            ...
                            <Link href={`/articles/${mainFeature.slug}`} className="cursor-pointer block">
                                <div className="relative mb-4 md:mb-6 overflow-hidden">
                                    ...
                                    <div className="absolute bottom-3 left-3 md:bottom-4 md:left-4">
                                        <span className="bg-secondary dark:bg-amber-500 text-on-secondary font-label-caps px-2.5 py-1 text-xs md:px-3 md:shadow-lg uppercase">
                                            TOP TRENDING #01
                                        </span>
                                    </div>
                                </div>
                                <div className="flex flex-col gap-3 md:gap-4">
                                    ...
                                    {/* Perbaikan ukuran judul utama mobile */}
                                    <h2 className="text-xl font-bold md:font-headline-md md:text-headline-md lg:font-headline-lg lg:text-headline-lg group-hover:text-secondary dark:group-hover:text-amber-500 transition-colors leading-tight uppercase">
                                        {mainFeature.title}
                                    </h2>
                                    <p className="text-on-surface-variant dark:text-on-primary-container text-sm md:font-body-lg max-w-3xl line-clamp-2">
                                        {mainFeature.excerpt}
                                    </p>
                    ...
                    {/* PILLAR LIST #02 - #04 */}
                    <section className="order-2 md:order-none md:col-span-4 border-r-0 md:border-l border-outline-variant pr-0 md:pr-gutter pb-6 md:pb-0 border-b md:border-b-0 mb-6 md:mb-0 pl:2">
                        {/* Mengurangi kerenggangan jarak antar list di mobile */}
                            ...
                            {leftPillar.map((item, index) => (
                                ...
                                >
                                    <div className="flex items-start gap-4">
                                        {/* Perbaikan ukuran nomor urut list mobile */}
                                        <span className="text-3xl font-extrabold text-primary dark:text-on-secondary opacity-20 dark:opacity-80 md:font-headline-md md:text-headline-md lg:font-display-xl lg:text-headline-lg leading-none">
                                            {String(index + 2).padStart(2, '0')}
                                        </span>
                                        <div>
                                            <span className="text-secondary dark:text-amber-500 font-label-caps mb-1 block text-xs uppercase">
                                                {item.category?.name}
                                            </span>
                                            {/* Perbaikan ukuran teks judul list mobile */}
                                            <h3 className="text-base font-bold md:font-headline-md md:text-headline-md leading-tight group-hover:text-secondary dark:group-hover:text-amber-500 transition-colors uppercase">
                                                {item.title}
                                            </h3>
                ...
                <div className="mt-8 md:mt-section-gap grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-gutter">
                    {bentoRow.map((item, index) => (
                        ...
                        >
                            <span className="text-2xl font-bold text-primary dark:text-on-secondary opacity-50 mb-1 md:mb-2 block md:font-headline-md md:text-headline-md lg:font-display-xl">
                                {String(index + 5).padStart(2, '0')}
                            </span>
                            <span className="text-secondary dark:text-amber-500 font-label-caps mb-1.5 md:mb-2 block text-xs uppercase">{item.category?.name}</span>
                            {/* Perbaikan teks judul kotak bento mobile */}
                            <h4 className="text-base font-bold md:font-headline-md md:text-headline-md mb-2 md:mb-4 leading-snug group-hover:text-secondary dark:group-hover:text-amber-500 transition-colors uppercase">
                                {item.title}
                            </h4>
                ...
                {/* Perbaikan padding section statistik mobile (dari p-margin-edge menjadi py-6 px-4) */}
                <section className="mt-8 md:mt-section-gap bg-primary text-on-primary py-6 px-4 md:p-margin-edge">
                    <div className="flex flex-col md:flex-row justify-between items-center gap-6 md:gap-8">
                        <div className="text-center md:text-left">
                            {/* Perbaikan teks judul statistik mobile */}
                            <h3 className="text-lg font-bold md:font-headline-md lg:font-headline-lg mb-1 md:mb-2 uppercase italic">Inside the Numbers</h3>
                            <p className="opacity-70 text-xs md:font-body-md italic">Real-time data from our global readership network.</p>
                        </div>
                        {/* Perbaikan grid kotak dan teks angka statistik mobile */}
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-6 md:gap-12 w-full md:w-auto">
                            {dynamicStats.map((stat, i) => (
                                <div key={i} className="text-center">
                                    <span className="text-xl font-bold block md:font-headline-md lg:font-display-xl lg:text-headline-lg">{stat.val}</span>

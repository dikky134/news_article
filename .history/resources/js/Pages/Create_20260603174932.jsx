...
export default function Create({ categories = [], article = null, drafts = [] }) {
    ...
    const Toast = Swal.mixin({
        ...
        background: '#1e1e1e', // Warna latar belakang gelap modern
        color: '#f3f4f6',      // Warna teks putih pudar
        didOpen: (toast) => {
    ...
    const { data, setData, post, put, processing, errors, reset } = useForm({
        ...
        is_draft: false, // Menambahkan parameter pembeda untuk backend
    });
    ...
    // Handler untuk Simpan sebagai Draf (Mengarahkan route dan flag is_draft)
        ...
        post('/articles', { 
            ...
            onSuccess: () => {
                ...
                Toast.fire({
                    ...
                    iconColor: '#38bdf8' // Warna biru info yang soft
                });
            },
        });
    };

    // Handler untuk Publish Artikel
            ...
            router.post(`/articles/${article.id}`, {
                ...
                onSuccess: () => {
                    Toast.fire({
                        ...
                        iconColor: '#4ade80' // Warna hijau sukses
                    });
                },
            });
        } else {
            ...
            <main className="max-w-content-max-width mx-auto px-4 md:px-margin-edge py-12 w-full overflow-x-hidden">
                ...
                <form onSubmit={handlePublish} className="grid grid-cols-1 lg:grid-cols-12 gap-gutter w-full">
                        ...
                        <section className="space-y-6">
                            ...
                            <div>
                                ...
                                <textarea 
                                    ...
                                    placeholder="Write a short summary of this article in 1-2 sentences (maximum 150 characters) to be displayed on the front page..."
                                />
                                {errors.subheadline && <p className="text-error text-xs mt-1">{errors.subheadline}</p>}
                            </div>
                            <div>
                                <label className="font-label-caps text-[11px] md:text-label-caps text-on-surface-variant mb-2 block uppercase tracking-widest">Excerpt</label>
                                <textarea 
                                    rows="2"
                                    value={data.excerpt}
                                    onChange={e => setData('subheadline', e.target.value)}
                                    className="w-full font-body-lg text-base md:text-body-lg bg-transparent border-b border-outline focus:border-primary focus:ring-0 px-0 py-2 resize-none transition-colors dark:text-white outline-none placeholder:text-sm" 
                                    placeholder="Enter 3-5 unique keywords separated by hyphens..."
                                />
                                {errors.excerpt && <p className="text-error text-xs mt-1">{errors.excerpt}</p>}
                            </div>
                        </section>
                        ...
                        <section className="w-full">
                            ...
                            <div className="border border-outline-variant bg-surface-container-lowest dark:bg-zinc-900 w-full">
                                <div className="flex flex-wrap items-center gap-3 md:gap-4 px-3 md:px-4 py-2.5 border-b border-outline-variant bg-surface-container-low dark:bg-zinc-800">
                                    ...
                                    <button type="button" onClick={() => formatText('quotes')} className="material-symbols-outlined text-[18px] md:text-[20px] text-on-surface-variant hover:text-primary dark:hover:text-white" title="Numbered List">format_quote</button>
                                    <div className="w-[1px] h-4 bg-outline-variant mx-0.5"></div>
                    ...
                    <aside className="lg:col-span-4 space-y-6 md:space-y-8 w-full">
                        ...
                        <div className="w-full lg:w-80 bg-[#1a1a1a] p-5 rounded-lg border border-neutral-800 h-fit">
                            <h3 className="text-sm font-mono tracking-wider uppercase text-amber-400 mb-4 flex items-center gap-2">
                                <span className="w-2 h-2 rounded-full bg-amber-500 animate-pulse"></span>
                                Draf Tersimpan ({drafts.length})
                            </h3>

                            {drafts.length === 0 ? (
                                <p className="text-xs text-gray-500 italic py-4 text-center">Belum ada draf.</p>
                            ) : (
                                <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
                                    {drafts.map((draft) => (
                                        <div 
                                            key={draft.id} 
                                            className="p-3 bg-[#242424] hover:bg-[#2a2a2a] rounded border border-neutral-800 transition flex flex-col justify-between"
                                        >
                                            <div className="mb-2">
                                                <h4 className="text-sm font-medium line-clamp-1 text-gray-200">
                                                    {draft.title || '(Belum ada judul)'}
                                                </h4>
                                            ...
                                            {/* Tombol untuk memuat ulang draf ke form */}
                                            <a 
                                                href={`/articles/${draft.id}/edit`}
                                                className="text-center px-3 py-1 bg-neutral-800 hover:bg-neutral-700 text-xs font-medium text-blue-400 rounded transition"
                                            >
                                                Buka Draf
                                            </a>
                                        </div>

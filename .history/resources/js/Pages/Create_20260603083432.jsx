...
export default function Create({ categories = [] }) {
    ...
    const formatText = (command) => {
        ...
        switch (command) {
            ...
            case 'link':
                ...
                if (url === null) return;
                formattedText = `<a href="${url}" target="_blank">${selectedText || 'Teks Link'}</a>`;
                break;
            case 'paragraph':
                formatText = `<p>${selectedText}</p>`;
                break;    
            default:
                return;
        }
    ...
    const handleAddCategory = (e) => {
        ...
        router.post('/categories', { name: newCategoryName }, {
            onSuccess: () => {
                setNewCategoryName(''); // Reset input text
                setCategoryError('');
            },
            onError: (err) => {
                setCategoryError(err.name || 'Gagal menambahkan kategori.');
            },
            preserveScroll: true, // Layar tidak otomatis scroll ke atas saat data bertambah
        });
    };
    ...
    const handleDeleteCategory = (id) => {
        if (confirm('Apakah Anda yakin ingin menghapus kategori ini?')) {
            // Gunakan console log untuk memastikan ID yang dikirim tidak undefined / kosong
                        ...
                        <section className="w-full">
                            ...
                            <div className="border border-outline-variant bg-surface-container-lowest dark:bg-zinc-900 w-full">
                                <div className="flex flex-wrap items-center gap-3 md:gap-4 px-3 md:px-4 py-2.5 border-b border-outline-variant bg-surface-container-low dark:bg-zinc-800">
                                    <button type="button" onClick={() => formatText('paragraph')} className="material-symbols-outlined text-[18px] md:text-[20px] text-on-surface-variant hover:text-primary dark:hover:text-white" title="Paragraph">format_paragraph</button>
                            ...
                            <div>
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <button 
                                        ...
                                        className="text-xs text-primary dark:text-amber-500 hover:underline"
                                    >
                                        {showManageCategory ? '✕ Close Quick Menu' : '⚙ Manage Categories'}
                                    </button>
                                </div>

                                {/* Dropdown Kategori Utama */}
                                <select 
                                    className="w-full border p-2 dark:bg-zinc-900 border-zinc-300 dark:border-zinc-700 uppercase text-xs"

                ...
                <header className="mb-12 border-b border-outline-variant pb-8">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="flex flex-col gap-4">
                            ...
                            {/* 1. TAMPILAN MOBILE (Dropdown <select> - Muncul di layar < md) */}
                            <div className="block md:hidden w-full">
                                <select
                                    // Mengambil nilai filter category yang sedang aktif di state/URL
                                    value={filters.category || ''}
                                    // Saat opsi diubah, panggil fungsi filter bawaan Anda
                                ...
                                >
                                    {/* Opsi default untuk reset filter */}
                                    <option value="">ALL TOPICS</option>
                                    
                                    {/* Looping kategori dinamis dari database phpMyAdmin */}
                            ...
                            {/* 2. TAMPILAN DESKTOP (Tombol Pills - Muncul di layar md ke atas) */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-gutter">
                            ...
                            {sideArticles.map((item) => (
                                <article key={item.id} className="group cursor-pointer border-b border-outline-variant pb-6">
                                    <Link href={`/articles/${item.slug}`}>
                                        {/* TAMBAHAN BARU: Gambar Pendukung yang muncul penuh di mobile, namun disembunyikan di desktop demi estetika layout Bento */}
                                        <div className="block md:hidden relative overflow-hidden border border-outline-variant/30 mb-4 aspect-[16/9] bg-surface-container">
                                            <img 
                                                className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700 scale-100 group-hover:scale-105" 
                                                src={item.thumbnail || 'https://via.placeholder.com/600xl350'} 
                                                alt={item.title}
                                            />
                                        </div>
                        ...
                        {bottomArticles.map((item) => (
                            <article key={item.id} className="md:col-span-4 group cursor-pointer mt-8">
                                <Link href={`/articles/${item.slug}`}>
                                    {/* Diperbarui dari aspect-square menjadi aspect-[16/9] di mobile agar serasi dan tidak terlalu memakan ruang tinggi layar handphone */}
                                    ...
                                    <h3 className="font-headline-md text-headline-md mb-3 group-hover:text-secondary dark:group-hover:text-amber-500 transition-colors uppercase">
                                        {item.title}
                                    </h3>
